...
const MONGO_URI = process.env.MONGO_URI_TELEMETRY || 'mongodb://localhost:27017/telemetrydb';
const PORT = process.env.PORT_TELEMETRY || 6005;

mongoose.connect(MONGO_URI)
  .then(() => console.log('✅ MongoDB (Telemetry) connected'))
  .catch(err => {
    console.error('❌ MongoDB (Telemetry) connection error:', err.message);
    process.exit(1);
  });
...
app.use(requestLogger('telemetry'));
...
app.get('/health', (_req, res) => {
  res.json({ service: 'telemetry', status: 'UP', timestamp: new Date().toISOString() });
});
app.get('/metrics', metricsRoute('telemetry'));
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Telemetry Service running on port ${PORT}`));
